...
class Validator( object ):
	...
	def __init__( self ):
		...
		self._new_amount_pattern = re.compile('^(((\\d{1},?\\d{3}|\\d{1,3})\\.\\d{2}|0{1}))|10\\,?000\\.00$')
	...
	def is_valid_amount( self, amount ):
		...
		if not m: raise ValueError('Amount is a positive number which cannot exceed $10,000 USD in any currency. It includes no currency symbol. It must have 2 decimal places, the decimal separator must be a period (.), and the optional thousands separator must be a comma (,).')
