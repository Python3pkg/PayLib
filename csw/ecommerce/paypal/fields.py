...
class BillingAgreementMulti( RequestFields ):
	...
	def set_billing_type( self, bill_type, payment_number ):
		...
		Note: Other defined values are not valid."""
		if bill_type not in billing_type:
			raise ValueError('bill_type must be in BillingAgreementMulti.billing_type')
		
		self._nvp_request[('L_BILLINGTYPE%d' % payment_number)] = billing_type[bill_type]
	...
	def set_ref_billing_type(self, bill_type):
		...
		self._nvp_request['BILLINGTYPE'] = billing_type[bill_type]
	...
	def set_payment_type( self, pay_type, payment_number ):
		...
		Note: For recurring payments, this field is ignored."""
		if pay_type not in payment_type:
			raise ValueError( 'pay_type must be in BillingAgreementMulti.payment_type' )
		self._nvp_request[('L_PAYMENTTYPE%d' % payment_number)] = payment_type[pay_type]
...
class PaymentItem( RequestFields ):
	...
	def set_tax_amount( self, amount ):
		...
		if not v.is_valid_amount( amount ):
			...
			raise ValueError( sb.getvalue() )
		if payment_number > 9:
			raise ValueError('Number cannot exceed 9')
	...
	def set_item_category(self, value):
		...
		val = value
		if val not in item_category:
			raise ValueError('Invalid item category')
		
		self._nvp_request['L_PAYMENTREQUEST_%d_ITEMCATEGORY%d'] = item_category[val]
...
class Payment( RequestFields ):
	...
	def set_payment_action(self, value):
		...
		if value not in payment_action:
			raise ValueError('Payment action value must exist in PaymentMulti.payment_action')
...
class ScheduleDetails( RequestFields ):
	...
	def set_max_failed_payments(number):
		self._nvp_request['MAXFAILEDPAYMENTS'] = str(number)
	
	def set_auto_bill_amount(autobill):
...
class BillingPeriodDetails( RequestFields ):
	...
	def __init__(self, period, frequency, amount, currency):
		...
		if period == 'Year' and frequency > 365:
			...
			raise ValueError('When billing period is set to Semi month, then billing frequecny has to be 1' % (billing_frequency,period))
...
class PayerInformation( RequestFields ):
	...
	def set_country(self, country):
		...
		self._nvp_request('COUNTRYCODE', country_dict[country])
...
country_dict = {'AFGHANISTAN':'AF','ALAND ISLANDS':'AX','ALBANIA':'AL','ALGERIA':'DZ','AMERICAN SAMOA':'AS','ANDORRA':'AD','ANGOLA':'AO','ANGUILLA':'AI','ANTARCTICA':'AQ','ANTIGUA AND BARBUDA':'AG','ARGENTINA':'AR',
				...
				'BERMUDA':'BM','BHUTAN':'BT','BOLIVIA':'BO','BOSNIA AND HERZEGOVINA':'BA','BOTSWANA':'BW','BOUVET ISLAND':'BV','BRAZIL':'BR','BRITISH INDIAN OCEAN TERRIRTORY':'IO','BRITISH VIRGIN ISLANDS':'VG',
				'BRUNEI':'BN','BULGARIA':'BG','BURKINA FASO':'BF','BURUNDI':'BI','CAMBODIA':'KH','CAMEROON':'CM','CANADA':'CA','CAPE VERDE':'CV','CAYMAN ISLANDS':'KY','CENTRAL AFRICAN REPUBLIC':'CF','CHAD':'TD',
				'CHILE':'CL','CHINA':'CN','CHRISTMAS ISLAND':'CX','COCOS (KEELING) ISLANDS':'CC','COLOMBIA':'CO','COMOROS':'KM','CONGO':'CG','GONGO, THE DEMOCRATIC REPUBLIC OF':'CD','COOK ISLANDS':'CK','COSTA RICA':'CR',
				'COTE D\'IVOIRE':'CI','CROATIA':'HR','CUBA':'CU','CYPRUS':'CY','CZECH REPUBLIC':'CZ','DENMARK':'DK','DJIBOUTI':'DJ','DOMINICA':'DM','DOMINICAN REPUBLIC':'DO','ECUADOR':'EC','EGYPT':'EG','EL SALVADOR':'SV',
				'EQUATORIAL GUINEA':'GQ','ERITREA':'ER','ESTONIA':'EE','ETHIOPIA':'ET','FALKLAND ISLANDS':'FK','FAROE ISLANDS':'FO','FIJI':'FJ','FINLAND':'FI','FRANCE':'FR','FRENCH GUIANA':'GF','FRENCH POLYNESIA':'PF',
				'FRENCH SOUTHERN TERRITORIES':'TF','GABON':'GA','GAMBIA':'GM','GEORGIA':'GE','GERMANY':'DE','GHANA':'GH','GIBRALTAR':'GI','GREECE':'GR','GREENLAND':'GL','GRENADA':'GD','GUADELOUPE':'GP',
				'GUAM':'GU','GUATEMALA':'GT','GUERNSEY':'GG','GUINEA':'GN','GUINEA-BISSAU':'GW','GUYANA':'GY','HAITI':'HT','HEARD ISLAND AND MCDONALD ISLANDS':'HM','VATICAN':'VA','HONDURAS':'HN',
				'HONG KONG':'HK','HUNGARY':'HU','ICELAND':'IS','INDIA':'IN','INDONESIA':'ID','IRAN':'IR','IRAQ':'IQ','REPUBLIC OF IRELAND':'IE','ISLE OF MAN':'IM','ISRAEL':'IL','ITALY':'IT','JAMAICA':'JM','JAPAN':'JP',
				'JERSEY':'JE','JORDAN':'JO','KAZAKHSTAN':'KZ','KENYA':'KE','KIRIBATI':'KI','NORTH KOREA':'KP','SOUTH KOREA':'KR','KUWAIT':'KW','KYRGYZSTAN':'KG','LAOS':'LA','LATVIA':'LV','LEBANON':'LB',
				'LESOTHO':'LS','LIBERIA':'LR','LIBYA':'LY','LIECHTENSTEIN':'LI','LITHUANIA':'LT','LUXEMBOURG':'LU','MACAO':'MO','MACEDONIA':'MK','MADAGASCAR':'MG','MALAWI':'MW','MALAYSIA':'MY','MALDIVES':'MV','MALI':'ML',
				'MALTA':'MT','MARSHALL ISLANDS':'MH','MARTINIQUE':'MQ','MAURITANIA':'MR','MAURITIUS':'MU','MAYOTTE':'YT','MEXICO':'MX','MICRONESIA':'FM','MOLDOVA':'MD','MONACO':'MC','MONGOLIA':'MN','MONTENEGRO':'ME',
				'MONTSERRAT':'MS','MOROCCO':'MA','MOZAMBIQUE':'MZ','MYANMAR':'MM','NAMIBIA':'NA','NAURU':'NR','NEPAL':'NP','NETHERLANDS':'ML','NETHERLANDS ANTILLES':'AN','NEW CALEDONIA':'NC','NEW ZEALAND':'NZ',
				'NICARAGUA':'NI','NIGER':'NE','NIGERIA':'NG','NIUE':'NU','NORFOLK ISLAND':'NF','NORTHERN MARIANA ISLANDS':'MP','NORWAY':'NO','OMAN':'OM','PAKISTAN':'PK','PALAU':'PW','PALESTINIAN TERRITORY':'PS',
				'PANAMA':'PA','PAPUA NEW GUINEA':'PG','PARAGUAY':'PY','PERU':'PE','PHILIPPINES':'PH','PITCAIRN':'PN','POLAND':'PL','PORTUGAL':'PT','PUERTO RICO':'PR','QATAR':'QA','REUNION':'RE','ROMANIA':'RO',
				'RUSSIA':'RU','RWANDA':'RW','SAINT HELENA':'SH','SAINT KITTS AND NEVIS':'KN','SAINT LUCIA':'LC','SAINT PIERRE AND MIQUELON':'PM','SAINT VINCENT AND THE GRENADINES':'VC','SAMOA':'WS','SAN MARINO':'SM',
				'SAO TOME AND PRINCIPE':'ST','SAUDI ARABIA':'SA','SENEGAL':'SN','SERBIA':'CS','SEYCHELLES':'SC','SIERRA LEONE':'SL','SINGAPORE':'SG','SLOVAKIA':'SK','SLOVENIA':'SI','SOLOMON ISLANDS':'SB',
				'SOMALIA':'SO','SOUTH AFRICA':'ZA','SOUTH GEORGIA AND THE SOUTH SANDWICH ISLANDS':'GS','SPAIN':'ES','SRI LANKA':'LK','SUDAN':'SD','SURINAME':'SR','SVALBARD AND JAN MAYEN':'SJ','SWAZILAND':'SZ',
				'SWEDEN':'SE','SWITZERLAND':'CH','SYRIA':'AY','TAIWAN':'TW','TAJIKISTAN':'TJ','TANZANIA':'TZ','THAILAND':'TH','EAST TIMOR':'TL','TOGO':'TG','TOKELAU':'TK','TONGA':'TO','TRINIDAD AND TOBAGO':'TT',
				'TUNISIA':'TN','TURKEY':'TN','TURKMENISTAN':'TM','TURKS AND CAICOS ISLANDS':'TC','TUVALU':'TV','UGANDA':'UG','UKRAINE':'UA','UNITED ARAB EMIRATES':'UA','UNITED KINGDOM':'UK','UNITED STATES':'US',
				'UNITED STATES MINOR OUTLYING ISLANDS':'UM','URUGUAY':'UY','US VIRGIN ISLANDS':'VI','UZBEKISTAN':'UZ','VANUATU':'VU','VENEZUELA':'VE','VIETNAM':'VN','WALLIS AND FUTUNA':'WF','WESTERN SAHARA':'EH',
				'YEMEN':'YE','ZAMBIA':'ZM','ZIMBABWE':'ZW'}
currency_set = set(['AUD','BRL','CAD','DKK','EUR','HKD','HUF','ILS','JPY','MYR','MXN','NOK','NZD','PHP','PLN','GBP','SGD','SEK','CHF','TWD','THB','USD'])
